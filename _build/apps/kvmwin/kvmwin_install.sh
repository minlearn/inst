...
silent apt-get install p7zip-full libarchive-tools wimtools genisoimage -y
...
wget --no-check-certificate $rlsmirror/virtio-win-1.9.57.tar.xz -O download/drivers.txz
...
# w7driver?
sed -e s#-no-emul-boot#-no-emul-boot[[:space:]]-boot-load-size[[:space:]]8#g -i /root/make.sh
sed -e s#-eltorito-alt-boot[[:space:]]-eltorito-boot[[:space:]]efi/microsoft/boot/efisys_noprompt.bin#d -i /root/make.sh
...
chmod +x /root/win7patcher.sh
    ...
    cat > /root/gpupather.sh << 'EOL'
# inject gpu for 3d rendering
EOL
chmod +x /root/gpupather.sh
