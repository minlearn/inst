...
wget --no-check-certificate https://snapshot.debian.org/archive/debian/20250306T150220Z/pool/main/l/linux/linux-config-6.1_6.1.129-1_amd64.deb -O download/mypackagecfg.deb
...
sed -e "s/#define[[:space:]]\+COMMAND_LINE_SIZE[[:space:]]\+2048/#define COMMAND_LINE_SIZE 20480/" -i mypackage/usr/src/linux-source-6.1/arch/x86/include/asm/setup.h
xz -dck mypackage/usr/src/linux-config-6.1/config.amd64_none_amd64.xz > mypackage/usr/src/linux-source-6.1/config.amd64_none_amd64
sed -e "s/CONFIG_DEBUG_INFO_BTF=y/CONFIG_DEBUG_INFO_BTF=n/g" -i mypackage/usr/src/linux-source-6.1/config.amd64_none_amd64

# sed -e "s/# CONFIG_IKCONFIG is not set/CONFIG_IKCONFIG=y\nCONFIG_IKCONFIG_PROC=y/g" -i mypackage/usr/src/linux-source-6.1/config.amd64_none_amd64
# sed -e "s/CONFIG_ANDROID_BINDER_IPC=m/CONFIG_ANDROID_BINDER_IPC=y/g" -e "s/# CONFIG_ANDROID_BINDERFS is not set/CONFIG_ANDROID_BINDERFS=y/g" -i mypackage/usr/src/linux-source-6.1/config.amd64_none_amd64
...
-i mypackage/usr/src/linux-source-6.1/config.amd64_none_amd64
BLOCK

sed \
-e '$a\CONFIG_BUILD_SALT="6.1.0-32-amd64"' \
...
-e 's/EXTRAVERSION =/EXTRAVERSION = -32-amd64/g' \
