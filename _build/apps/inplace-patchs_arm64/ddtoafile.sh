slipgrubcfgs(){
  cat > $chrootdir2/boot/grub/grub.cfg <<'EOF'
...
submenu 'start devdeskos recovery' --users admin $menuentry_id_option 'gnulinux-advanced-BOOTFSUUID' {
	...
	menuentry 'reinstall devdeskos' --class debian --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-5.10.0-22-arm64-advanced-BOOTFSUUID' {
		load_video
		insmod gzio
		if [ x$grub_platform = xxen ]; then insmod xzio; insmod lzopio; fi
		load_sas
	...
	menuentry 'erase data volume' --class debian --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-5.10.0-22-arm64-recovery-BOOTFSUUID' {
		load_video
		insmod gzio
		if [ x$grub_platform = xxen ]; then insmod xzio; insmod lzopio; fi
		load_sas
...
EOF
  cat > $chrootdir2/boot/EFI/boot/grub.cfg <<'EOF'
...
#########edtion2
  ...
  chrootdir2=$topdir/$remasteringdir/onekeydevdeskd_arm64/01-core

  mkdir -p $chrootdir2/boot/grub $chrootdir2/boot/EFI/boot

  ar -p $topdir/$downdir/debianbase/dists/bullseye/main/binary-arm64/deb/grub-efi-arm64-bin_2.06-3~deb11u5_arm64.deb data.tar.xz |xzcat|tar -xf - -C $chrootdir2/boot ./usr/lib/grub/ --strip-components=3

  mkdir -p $chrootdir2/boot/grub/fonts
  ar -p $topdir/$downdir/debianbase/dists/bullseye/main/binary-arm64/deb/grub-common_2.06-3~deb11u5_arm64.deb data.tar.xz |xzcat|tar -xf - -C $chrootdir2/boot/grub/fonts ./usr/share/grub/unicode.pf2 --strip-components=4
  slipgrubcfgs
