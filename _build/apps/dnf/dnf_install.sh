...
# preprocess
for SQL_FILE in ${DNFS_DIR}/database/*.sql
  do
    TARGET_DB=\$(basename "\$SQL_FILE" .sql)
    sed -i "/CREATE DATABASE.*\\\`\${TARGET_DB}\\\`/d; /USE \\\`\${TARGET_DB}\\\`;/d" "\$SQL_FILE"
  done

MAIN_DB_LIST=("d_taiwan" "d_taiwan_secu" "d_technical_report" "d_guild" "taiwan_mng_manager" "taiwan_login" "taiwan_prod" "taiwan_game_event" "taiwan_se_event" "taiwan_login_play" "taiwan_billing")
SG_DB_LIST=("d_channel_${SERVER_GROUP_DB}" "taiwan_${SERVER_GROUP_DB}" "taiwan_${SERVER_GROUP_DB}_2nd" "taiwan_${SERVER_GROUP_DB}_log" "taiwan_${SERVER_GROUP_DB}_web" "taiwan_${SERVER_GROUP_DB}_auction_gold" "taiwan_${SERVER_GROUP_DB}_auction_cera")
...
CREATE USER 'game'@'127.0.0.1' IDENTIFIED BY '\$GAMEUSERPASSWORD';
CREATE USER 'game'@'localhost' IDENTIFIED BY '\$GAMEUSERPASSWORD';
CREATE USER 'game'@'\${HOSTNAME%.*.*}.%.%' IDENTIFIED BY '\$GAMEUSERPASSWORD';
"
...
for db in "\${MAIN_DB_LIST[@]}" "\${SG_DB_LIST[@]}"; do
  mysql -h\${HOSTNAME} -P\${PORT} -u\${USERNAME} -p\${pw} -e "
  GRANT ALL ON \$db.* TO 'game'@'127.0.0.1';
  GRANT ALL ON \$db.* TO 'game'@'localhost';
...
# no needed: will update autoly by game server
...
mysql -h\${HOSTNAME} -P\${PORT} -u\${USERNAME} -p\${pw} <<EOFEOF
CREATE TABLE IF NOT EXISTS taiwan_${SERVER_GROUP_DB}_auction_cera.auction_history_\${dt} LIKE taiwan_${SERVER_GROUP_DB}_auction_cera.auction_history_202212;
CREATE TABLE IF NOT EXISTS taiwan_${SERVER_GROUP_DB}_auction_cera.auction_history_buyer_\${dt} LIKE taiwan_${SERVER_GROUP_DB}_auction_cera.auction_history_buyer_202212;
CREATE TABLE IF NOT EXISTS taiwan_${SERVER_GROUP_DB}_auction_gold.auction_history_\${dt} LIKE taiwan_${SERVER_GROUP_DB}_auction_gold.auction_history_202212;
CREATE TABLE IF NOT EXISTS taiwan_${SERVER_GROUP_DB}_auction_gold.auction_history_buyer_\${dt} LIKE taiwan_${SERVER_GROUP_DB}_auction_gold.auction_history_buyer_202212;
EOFEOF
done
...
find ${DNFS_DIR}/build -type f -name "*.cfg" -exec sh -c 'grep -q 3307 "\$0" || sed -i "s/game_db_ip = 127\\.0\\.0\\.1/game_db_ip = '"\${HOSTNAME}"'/g" "\$0"' {} \\;
