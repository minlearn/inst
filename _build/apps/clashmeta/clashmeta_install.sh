...
test -f /root/inited || {
  echo -e "选择导入方式：\n1) 订阅URL（自动截取proxies段，沿用订阅自带groups/rules）\n2) 手动输入单个VMess（自动补齐proxies/proxy-groups/rules顶层）"
  read -p "请选择(1/2): " mode
  ...
  if [ "$mode" = "1" ]; then
    ...
    read -p "give a VMess server: " server
    read -p "give a VMess port: " port
    read -p "give a VMess uuid: " uuid
...
echo "仅标记rdp流量特征并加速，to be implement"
exit 0
EOL
