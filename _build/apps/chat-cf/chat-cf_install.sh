...
silent npm install -g wrangler@4.32.0

rlsmirror=${2:-https://github.com/minlearn/inst/releases/download/inital}
mkdir -p /app/chat/dist
wget --no-check-certificate ${rlsmirror}/chat-cf.tar.gz -O /tmp/apps.tar.gz
tar -xzvf /tmp/apps.tar.gz -C /app/chat/dist ./
rm -rf /tmp/apps.tar.gz
   
cat > /app/chat/wrangler.toml << 'EOL'
compatibility_date = "2024-03-07"
[[d1_databases]]
binding = "chatdb"
database_name = "chat_chatdb_development"
database_id = "11111111-1111-1111-1111-111111111111"
[durable_objects]
bindings = [
  { name = "STATE", class_name = "StateDO" }
]
[site]
bucket = "./"
EOL

#cat > /lib/systemd/system/chatd1.service << 'EOL'
#[Unit]
...
#Type=oneshot
#ExecStart=wrangler d1 execute chat_chatdb_development --file=dist/db.sql --local
...
cat > /lib/systemd/system/chat.service << 'EOL'
[Unit]
...
if [[ ${prompt,,} =~ ^(y|yes)$ ]]; then
  ...
  sed -e s/0.0.0.0/127.0.0.1/g -i /lib/systemd/system/chat.service
