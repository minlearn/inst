...
DEBVER=`echo "$corefiles" | awk -F ',' '{ print $4}'`
...
dobootstrap(){
  ...
  mkdir -p p4/down;
  for i in grub-common_2.06-3-deb11u5_arm64.deb grub-efi-arm64-bin_2.06-3-deb11u5_arm64.deb; do
  ...
  ar -p p4/down/grub-common_2.06-3-deb11u5_arm64.deb data.tar.xz |xzcat|tar -xf - -C p4/boot/grub ./usr/bin/grub-mkimage --strip-components=3
...
post(){
              ...
              if [ -d "$therealospath/etc/NetworkManager/system-connections" ] && ! ls $therealospath/etc/NetworkManager/system-connections/Wired\ connection\ 1* 1>/dev/null 2>&1; then [ "$staticnetinfo" != '' -a "$ISDHCP" != '1' ] && { printf "[connection]\nid=Wired connection 1\nuuid=e5adadaf-db92-4085-a3a5-c1310a3c88dd\ntype=ethernet\nautoconnect=true\n\n[ethernet]\n\n[ipv4]\naddress1=$IP/$CIDR11,$GATE\ndns=8.8.8.8;\nignore-auto-dns=true\nmethod=manual\n\n[ipv6]\nip6-privacy=2\nmethod=auto\n\n[proxy] " > $therealospath/etc/NetworkManager/system-connections/Wired\ connection\ 1;chmod 600 $therealospath/etc/NetworkManager/system-connections/Wired\ connection\ 1;chown root:root $therealospath/etc/NetworkManager/system-connections/Wired\ connection\ 1; }; fi
