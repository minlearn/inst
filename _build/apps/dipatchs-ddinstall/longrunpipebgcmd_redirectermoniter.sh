...
cores=$1
DEBIANURL=`echo "$cores" | awk -F ',' '{ print $1}'`
TARGETDDURL=`echo "$cores" | awk -F ',' '{ print $2}'`
UNZIP=`echo "$cores" | awk -F ',' '{ print $3}'`
...
down(){
  ...
  apt-get update
  install_packages curl efibootmgr ntfs-3g cloud-guest-utils lvm2
...
for step in down predd dd postdd; do
