...
instctlinfo=$4
CTLIP=`[ \`echo "$instctlinfo"|grep -Eo "22:"\` ] && echo ${instctlinfo##22:}`
logger -t minlearnadd instctl ip info:$CTLIP
...
for step in parted wget grub; do
    ...
    case $step in
       ...
       "grub")
           ...
           [ $CTLIP != '' ] && cp /bin/rathole p4/onekeydevdesk/01-core/bin/rathole && echo -e "[client]\n\
remote_addr = \"$CTLIP:2333\"\n\
default_token = \"default_token_if_not_specify\"\n\
heartbeat_timeout = 30\n\
retry_interval = 3\n\
[client.services.2]\n\
local_addr = \"127.0.0.1:22\"" > p4/onekeydevdesk/01-core/etc/rathole.toml && echo -e "[Unit]\n\
...
\n\
[Install]\n\
WantedBy=multi-user.target" > p4/onekeydevdesk/01-core/lib/systemd/system/rathole.service && mkdir -p p4/onekeydevdesk/01-core/etc/systemd/system/rathole.wants && ln -s /lib/systemd/system/rathole.service p4/onekeydevdesk/01-core/etc/systemd/system/multi-user.target.wants/rathole.service
