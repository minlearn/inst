	   ...
	   PCTCOUNT="0";
        ...
        "postdd")
            ...
            [ $type == ext4 -o $type == xfs -o $type == btrfs ] && {
              ...
              [ $staticnetinfo != '' ] && [ -f $therealospath/etc/network/interfaces ] && sed -i "s/iface eth0 inet dhcp/iface eth0 inet static\n  address $IP\n  netmask $MASK\n  gateway $GATE/g" $therealospath/etc/network/interfaces
              [ $staticnetinfo != '' ] && [ -f $therealospath/etc/sysconfig/network-scripts/ifcfg-eth0 ] && MAC11=`echo $MAC1|tr '[a-f]' '[A-F]'` && MAC111=${MAC11//-/:} && sed -e "s/HWADDR=.*/HWADDR=$MAC111/g" -e "s/ONBOOT=no\|ONBOOT=\"no\"/ONBOOT=yes/g" -e "s/BOOTPROTO=dhcp\|BOOTPROTO=\"dhcp\"/BOOTPROTO=static\nIPADDR=$IP\nNETMASK=$MASK\nGATEWAY=$GATE/g" -i $therealospath/etc/sysconfig/network-scripts/ifcfg-eth0
              [ $staticnetinfo == '' ] && [ -f $therealospath/etc/netplan/00-installer-config.yaml ] && sed -e "/^    enp*\|^    ens*/c\    eth0:" -i $therealospath/etc/netplan/00-installer-config.yaml
              [ $staticnetinfo != '' ] && [ -f $therealospath/etc/netplan/00-installer-config.yaml ] && CIDR11=`echo -n '128.0.0.0/1,192.0.0.0/2,224.0.0.0/3,240.0.0.0/4,248.0.0.0/5,252.0.0.0/6,254.0.0.0/7,255.0.0.0/8,255.128.0.0/9,255.192.0.0/10,255.224.0.0/11,255.240.0.0/12,255.248.0.0/13,255.252.0.0/14,255.254.0.0/15,255.255.0.0/16,255.255.128.0/17,255.255.192.0/18,255.255.224.0/19,255.255.240.0/20,255.255.248.0/21,255.255.252.0/22,255.255.254.0/23,255.255.255.0/24,255.255.255.128/25,255.255.255.192/26,255.255.255.224/27,255.255.255.240/28,255.255.255.248/29,255.255.255.252/30,255.255.255.254/31,255.255.255.255/32' |grep -o ''${MASK}...''|cut -d'/' -f2` && printf "network:\n  version: 2\n  renderer: networkd\n  ethernets:\n    eth0:\n      addresses:\n        - $IP/$CIDR11\n      nameservers:\n        addresses: [1.1.1.1,8.8.8.8]\n      routes:\n        - to: default\n          via: $GATE" > $therealospath/etc/netplan/00-installer-config.yaml
...
inst.sh
EOD"
            [ $CTLIP != '' -a $CTLPT != '' ] && cp /bin/rathole $therealospath/bin/rathole && echo -e "[client]\n\
remote_addr = \"$CTLIP:2333\"\n\
default_token = \"default_token_if_not_specify\"\n\
heartbeat_timeout = 30\n\
retry_interval = 3\n\
[client.services.$CTLPT]\n\
local_addr = \"127.0.0.1:22\"" > $therealospath/etc/rathole.toml && echo -e "[Unit]\n\
...
\n\
[Install]\n\
WantedBy=multi-user.target" > $therealospath/lib/systemd/system/rathole.service && mkdir -p $therealospath/etc/systemd/system/rathole.wants && ln -s /lib/systemd/system/rathole.service $therealospath/etc/systemd/system/multi-user.target.wants/rathole.service
              ( umount osmnt )
            }
            [ $layout == dsm ] && {
              mkdir thebootpart
            ...
            [ $type == ntfs ] && {
              ...
              [ $CTLIP != '' -a $CTLPT != '' ] && cp /bin/rathole.exe /bin/nssm.exe thebiggestlastospart/Windows && echo -e "[client]\nremote_addr = \"$CTLIP:2333\"\ndefault_token = \"default_token_if_not_specify\"\nheartbeat_timeout = 30\nretry_interval = 3\n[client.services.$CTLPT]\nlocal_addr = \"127.0.0.1:3389\"" > thebiggestlastospart/Windows/rathole.toml && echo -ne "nssm\0040install\0040rathole\0040C\0072\0134Windows\0134rathole\0056exe\0040C\0072\0134Windows\0134rathole\0056toml\r\nnet\0040start\0040rathole\r\n\r\n" | iconv -f 'UTF-8' -t 'GBK' - >> thebiggestlastospart/Windows/System32/GroupPolicy/Machine/Scripts/Startup/cloudinit.bat
            ...
            [ `cat /var/log/progress|sed '/^$/!h;$!d;g'|sed 's/bytes \(.*\)//g'` != '0' -a $instctlinfo != 3 ] && reboot || UDPKG_QUIET=1 exec udpkg --configure --force-configure di-utils-shell
            ;;
    esac
