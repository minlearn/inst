...
inst: 省事好用的终身linux和一键安装脚本/面板/商店(lifelong linux and onekeystoke netinstall script/panel/appstore)🚀🚀🎉🎉
=====

inst是一套可在线一键安装os的脚本/面板和应用商店，及一套完整的linux定制和安装/重装框架和方案。   

 * 作为核心provision引擎部分，基于增强的debianinstaller和嵌入的pve lxc环境,inst.sh可一键为你的日常os和devdesk提供正反向新系统生成/重装支持。 
 * 作为前端增强部分，安装后的系统尽量搭配使用kasmvnc实现开箱即用。    
 * 作为脚本应用和自定义app商店部分，inst.sh可以为这些系统安装各种常见应用和隔离开发环境。  

> inst也是一整套完整的linux自建实验和编程教学方案：比如它源码开放且raw风格书写，且尽量不引用外部资源，抽象适中,扩展丰富，适合自建和用户定制,etc ..
...
inst支持一键dd其它多种os和为这些os(目前仅debian和devdesk)注入多种应用，配合这些应用，可以将这些os做成路由器，做成programmable nas，，做成sidebar os, 做成青龙，做成allinone,做成btpanel,,做成安卓群控,做成devops,做成serverless cli  
...
> 也可以面板上直选完成，推荐新手使用面板，更方便快捷  
> 安装前，关掉设备的secure boot及一切有可能阻止安装的硬件保护  
...
> 恢复完成后的系统，```一般地，使用iso安装或源安装的linux/windows其root/administrator密码都是inst.sh，iso包安装的桌面linu会多增加一个用户名为user密码为inst.sh的用户,使用自带密码的dd包(比如gz/xz/qcow2)安装的linux/windows会保留原包密码```，也可指定密码安装，指定密码的时候：注意windows指定时密码小于6位或8位可能不符合要求会导致密码无效   
> pve安装完端口是8000不是8006,本人不持有不保留对pve相关的增强和修改方面的任何授权，一切归官方即商标注册权方所有。    
...
| osx11            | apple        | 10-15GB  | amd64 | UEFI        | none          | [macos-bigsur-11.7.11-20g1443.dmg](https://guest:guest@d.minlearn.org/dav/osx11/macos-bigsur-11.7.11-20g1443.dmg_000:https://guest:guest@d.minlearn.org/dav/osxp2/macos-bigsur-11.7.11-20g1443.dmg_001) |
| osx12            | apple        | 10-15GB  | amd64 | UEFI        | none          | [macos-monterey-12.7.6-21h1320.dmg](https://guest:guest@d.minlearn.org/dav/osx12/macos-monterey-12.7.6-21h1320.dmg_000:https://guest:guest@d.minlearn.org/dav/osxp2/macos-monterey-12.7.6-21h1320.dmg_001) |
 ...
 * fork本仓库，然后修改你fork到仓库的inst.sh头部变量定义区的FORCEREPOMIRROR赋值为你仓库的对应对址，用 "https://github.com/你的github用户名/inst/raw/master/inst.sh" 脚本地址调用命令，你也可以用加"-m 你的新repomirror"参数的方式调用命令  
 * 按_build/inst已有samples，编写自己的安装逻辑并调用。  
 * 参照_build/appp每个app的目录结构，自己增加app脚本和逻辑  
 * 如果你仅想自托管inst.sh，那么上面足够，如果你还想托管panel，那么你需要在_build/www/panel目录下的所有文件部署到你的cf上，记得改一下wrangler.jsonc中的路径。  
...
`怎么联系: 点击如下作者tg地址，或进minlearn.org点击网页右下端聊天悬钮，简单说明需求或说明来意即可，不要说你好，在吗。直接说事`  
[@minlearn_supportly_bot](https://t.me/minlearn_supportly_bot)
...
![](https://raw.githubusercontent.com/minlearn/inst/master/_build/www/assets/donate.png)
...
此项目关联 https://github.com/minlearn/ 下所有项目，主体为 https://github.com/minlearn/minlearnprogramming/ 和 https://github.com/minlearn/minlinux ，这是一套为配合我在《minlearnprogramming》最小编程/统一开发的想法的辅助项目: 即最简单实用的终身linux和多开方案。  
本项目长期保存
