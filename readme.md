 ...
 * 作为核心provision引擎部分，基于增强的debianinstaller和嵌入的pve lxc环境,inst.sh可一键为你的日常os和devdesk提供新系统生成/重装支持。 
 * 作为前端增强部分，安装后的系统尽量搭配使用kasmvnc实现开箱即用。    
 * 作为脚本应用和自定义app商店部分，inst.sh可以为这些系统安装各种常见应用和隔离开发环境。  

> inst也是一整套完整的自建实验和编程教学方案：比如它源码开放且raw风格书写，且尽量不引用外部资源，抽象适中,扩展丰富，适合自建和用户定制,etc ..
